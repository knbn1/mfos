...
if not exist "%usrmods%/devtools.mfm" (set "version=2026.04.25")
if exist "%usrmods%/devtools.mfm" (set "version=2026.04.25-dev")
...
echo [bootloader] INFO: log location: %logfile%
echo.
...
echo [kernel] INFO: system disk is "%disk0label%" >> "%logfile%"
) else (
...
echo Memory sector 1 - Core system files>"%devices%/mem/memsect1"
...
if exist "%usrmods%/devtools.mfm" (if exist "%usrmods%/flashbreak.mfm" (
  ...
  echo [flashbreak-stage-2] INFO: starting sysmodule patches... >> "%logfile%"
...
echo [kernel-device-init] ERROR: failed to initialize userdata partition >> "%logfile%"
echo.
...
echo [kernel-userdata-init] ERROR: no userdata found for user %username% >> "%logfile%"
echo.
...
echo.
if exist "%usrmods%/devtools.mfm" (if exist "%usrmods%/flashbreak.mfm" (echo F145HBR34K %fbver% && echo.))
if not exist "%usrdir%" (
echo Userdata for user %username% not found!
...
title %titlebar%
if exist "%usrmods%/devtools.mfm" (if exist "%usrmods%/flashbreak.mfm" (title %titlebar% [F145HBR34K %fbver%] && echo [flashbreak] INFO: modified titlebar >> "%logfile%"))
...
if exist "%usrmods%/devtools.mfm" (if exist "%usrmods%/flashbreak.mfm" (echo F145HBR34K version: %fbver% && echo [about] INFO: flashbreak version is %version% >> "%logfile%") )
...
if exist "%usrmods%/flashbreak.mfm" (echo Failed to delete F145HBR34K sysmodule! && echo [flashbreak] ERROR: could not delete flashbreak sysmodule! >> "%logfile%" && goto prompt)
